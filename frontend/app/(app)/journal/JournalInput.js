...
export default function JournalInput() {
  ...
  return (
    <section className="p-6 pt-64 h-svh relative flex flex-col items-center" onClick={hideHint} onDragStart={hideHint} onTouchStart={hideHint} onScroll={hideHint}>
      <h1 className="text-center font-semibold mb-6">Daily Journal</h1>
      <textarea className="w-full text-xl border-none focus:outline-none resize-none h-96 overflow-auto [&::-webkit-scrollbar]:hidden entry" value={entry} onChange={(e) => setEntry(e.target.value)} id="entry" />
      {entry === '' ?
        <MessagePlaceholder ui={ui} /> : null
      }

      <AnimatePresence>
        {entry != '' ?
          <motion.button {...motionProps(0)} type="button" className="bg-yellow-950 text-white px-6 py-4 font-bold rounded-full">Log Entry</motion.button> : null
        }
      </AnimatePresence>
      {entry === '' ?
        <GestureHint showGestureHint={showGestureHint} setShowGestureHint={setShowGestureHint} top={60}>
          <div className="text-yellow-950/20 animate-pulse gap-3 flex flex-col items-center">
            ...
            <p><span className="md:hidden">Swipe</span><span className="hidden md:inline-block">Scroll</span> to see past entries
            </p>
          </div>
        </GestureHint>
        : null
