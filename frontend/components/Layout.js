...
export default function Layout({ route, fullWidth, children, onClick, className }) {
  ...
  return (
    <main className={`${fullWidth ? 'w-full' : 'max-w-sm'} mx-auto ${className}`} onClick={onClick} onDragStart={onClick} onTouchStart={onClick}>
      <motion.header {...motionProps(0)} className="fixed z-50 top-12 left-0 right-0 flex justify-center items-center max-w-sm mx-auto">
        <Link href='/home'>
          <Image src={logo} className="w-14 h-14" alt="Daily Dose Happy Emoticon" />
        </Link>
      </motion.header>
      {children}
      <motion.div className="fixed z-50 top-12 lg:bottom-12 right-12 drop-shadow-xl rounded-full backdrop-blur-xl" {...motionProps(1)}>
